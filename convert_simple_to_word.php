...
// Lire le contenu du fichier Markdown simplifié
$markdownContent = file_get_contents('Rapport_Tests_BTP_Manager_Simple.md');
...
foreach ($lines as $line) {
    ...
    if (preg_match('/^```(.+)?$/', $line)) {
        if ($inCodeBlock) {
            // Fin du bloc de code
            $section->addText($codeContent, $codeStyle);
            $section->addTextBreak();
            $inCodeBlock = false;
            $codeContent = '';
        } else {
...
$objWriter->save('Rapport_Tests_BTP_Manager_Simple.docx');

echo "Rapport Word simplifié généré avec succès : Rapport_Tests_BTP_Manager_Simple.docx\n";
